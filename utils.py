...
def equals(loc1, loc2) -> bool:
    ...
    if math.abs(pythagoras(loc1[0], loc1[1], loc2[0], loc2[1])) <= 50:
        return True
    return False


def generate_plane(id) -> Plane:
    # randomly generates a plane
    heading = np.random.rand() * 360
    plane = Plane(id, heading)
    return plane
...
def generate_spots() -> list[HoldingLoc]:
    ...
    while generative_radius < (ATC_RADIUS - 2000):  # subtract 2000 since we need to leave access ring on the outside
                ...
                if double == 1:
                    ...
                    for i in range(-1, 0):
                        for j in range(-1, 0):
                            ...
                            spots.append(spot)
                            lanes.append(
                                spot)  # this should not be needed in the end, but is an optimization that we make
                    double = 0;
                else:
                    for i in range(-1, 0):
                        for j in range(-1, 0):
        ...
        # adding special cells in front of runway. Not even showing up in holding cells, only in lanes.
        # allows planes to navigate through quadrants if we time and schedule properly
        x = math.cos(theta)
        y = math.sin(theta)
        for i in range(-1, 0):
            for j in range(-1, 0):
...
def instructions_to_land(plane, runway, lanes):
    ...
    targ_loc = 0, 0
    if pythagoras(runway.get_centerline(), runway.get_northtarget(), plane.get_location[0],
                  plane.get_location[1]) < pythagoras(runway.get_centerline(), runway.get_southtarget(),
                                                      plane.get_location[0], plane.get_location[1]):
...
def find_nearest_spot(cells: list[HoldingLoc], id):
    for cell in cells:
        # by nature of our cell generation, we will find the nearest cell and go towards it.
        if cell.is_parkable and cell.is_vacant:
            cell.set_full(id)
            return cell
